pub mod l3_compiler {
    ...
    #[derive(Debug, Clone)]
    pub(crate) struct Add<E1: Step<i64>, E2: Step<i64>> {
        pub(crate) e1: E1,
        pub(crate) e2: E2,
    }
    ...
    pub fn test_compiler_error(arg: i64) -> i64 {
        let arg1 = arg;
        let arg2 = true;

        // with a lax type system, you can see this will return arg+1
        // however, arg1 and arg2 are different types, so
        // if _ then arg1 else arg2
        // is not well typed!
        // however, this is difficult for the parser!compiler to tell

        let l1 = Location::from(0);

        // let main = L3! {
        //     l1 := (if true then arg1 else arg2); (if arg2 then 1 else arg1+1)
        // };
        // let main = L3! {
        //     if (let val x: int = 1 in x end) then 1 else 1
        // };
        // main.step()
        1
    }
